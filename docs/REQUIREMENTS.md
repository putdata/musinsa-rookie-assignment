...
#### 수강신청 정책
- **결정**: 신청/취소만 지원. 수강 변경(교체)은 취소 후 재신청으로 처리. 재수강 제한 없음. 취소 기한 제한 없음.
- **근거**: 기획팀이 "수강신청"과 "수강취소"만 명시. 복잡한 정책(변경, 재수강 제한, 취소 기한)은 명시되지 않았으므로 단순하게 구현. 실제 운영 시에는 수강 변경 기간, 취소 기한 등의 추가 정책이 필요할 수 있으며, 이는 Enrollment 엔티티에 상태(status) 필드를 추가하여 확장 가능.
...
- **데드락 방지**: 락 획득 순서를 Student → Course 순으로 고정하여 데드락 회피.
