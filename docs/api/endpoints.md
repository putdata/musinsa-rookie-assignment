...
| 400 | 잘못된 요청 | 학점 초과, 잘못된 파라미터, 입력 검증 실패 |
...
잘못된 요청 본문이나 파라미터 전송 시 통합 에러 형식으로 응답합니다.

| 상황 | HTTP 상태 | 에러 코드 | 메시지 예시 |
|------|----------|----------|-----------|
| 필수 필드 누락 (null) | 400 | INVALID_REQUEST | 잘못된 요청입니다. |
| 경로 파라미터 타입 오류 (문자열 등) | 400 | INVALID_REQUEST | 잘못된 요청입니다. |
| 잘못된 JSON 형식 | 400 | INVALID_REQUEST | 잘못된 요청입니다. |

### 정렬 기준

모든 목록 조회 API는 **ID 오름차순**으로 정렬되어 반환됩니다.
...
본문 없음. 동시 요청 시에도 Enrollment 비관적 락을 통해 enrolled 카운터의 이중 차감을 방지합니다 (ADR-003).

**에러 응답**

| 상황 | HTTP 상태 | 에러 코드 | 메시지 |
|------|----------|----------|--------|
| 수강신청 미존재 | 404 | ENROLLMENT_NOT_FOUND | 수강신청 내역을 찾을 수 없습니다. |
