...
- **3,000 VU**까지 무장애 안정 동작, HTTP 에러율 **0.00%**

#### 병렬 처리 개선 효과 (3,000 VU 기준)

| 지표 | 순차 처리 | 병렬 20스레드 | 변화 |
|------|----------|-------------|------|
...
#### 스트레스 테스트 (병렬 처리, 1,000 ~ 5,000 VU)

| 지표 | 1,000 VU | 3,000 VU | 5,000 VU |
|------|----------|----------|----------|
| queue_wait avg | 4.2s | **5.9s** | 9.9s |
| queue_wait p95 | 9.0s | **8.0s** | 11.5s |
| enroll_success | 2,018 | **2,018** | 2,018 |
| checks_failed | 0.00% | **0.00%** | 11.2%* |

\* 5,000 VU의 checks_failed는 단일 머신 TCP 연결 한계. 서버 도달한 요청은 100% 정상 처리.
...
> Phase 3은 별도 k6 시나리오(queue-enrollment-rush, 1000 VU)로 측정하여 Phase 0~2와 직접 수치 비교는 제한적입니다. 핵심은 아키텍처 전환에 따른 근본적 변화입니다.
...
- **Phase 3**: http_req_failed **0.00%** — 결과가 HTTP 에러가 아닌 데이터(SUCCESS/FAILED)로 전달

#### 사용자 체감 대기 시간
- p95 응답시간: **2.49ms** (HTTP 응답 기준)
- 대기열 처리 대기: avg **4.2초**, p95 **9.0초**
...
Phase 3 (대기열)      p95  2.49ms ── HTTP 에러 0%, 동시접속 2배
                                     │ 병렬 처리 (20스레드): queue_wait -57%
Phase 3 (병렬)        3,000 VU 안정 ── queue_wait avg 5.9s, 처리량 2배
```
